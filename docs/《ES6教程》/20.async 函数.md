...
> 关于执行顺序：
>
> 1.同一代码块中使用await为继发操作；可以用变量转换一遍或Promise.all来实现并发。
>
> 2.循环中，for循环为继发操作(for (let doc of docs){})；forEach或map等迭代为并发操作。
...
[2022](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Ftc39%2Fproposal-error-cause)最新提案在Error构造函数新增了一个可选参数cause，允许我们在实例化Error时，将错误原因以参数形式传入，省去了我们自己单独处理的成本。
...
### [语法](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/for-await...of#语法)
...
> 值得一提的是：官方说明了不能在 map/forEach 中使用异步
