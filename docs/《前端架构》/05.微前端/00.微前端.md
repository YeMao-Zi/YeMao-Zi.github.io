...
> 也就是说一个子应用对应一个iframe，但这个iframe是隐藏的，会把子应用地址获取到的资源提取script（处理过）插入这个iframe中，然后把元素和样式挂载到 Shadow DOM 上，创建一个代理对象，代理iframe中的dom操作，执行dom操作时变为操作 Shadow DOM,然后通过 postMessage 进行跨域通信和数据同步。
