...
提取 theme 的消费 token ,将其转化为 css 变量并写入 root 中
 ...
 */
export function generateThemeVariables(token, prefixCls) {
  const cssVar = Object.entries(token)
    ...
    .map(([key, value]) => `--${prefixCls}-${lodash.kebabCase(key)}: ${value}${/(size|width|radius|margin|screen|padding)/ig.test(key) ? 'px' : ''}`)
...
watchImmediate(token, (token) => (css.value = generateThemeVariables(token, 'antd')));
</script>
