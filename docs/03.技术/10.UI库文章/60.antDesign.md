...
import { Form, Input, Button, Space, type FormProps, type FormInstance } from "antd";

export interface formItem {
  itemProps: { label: string; name: string };
...
const MyForm = forwardRef(
  (
    ...
    const handleReset = () => {
      const Ref = ref || formRef;
      (Ref as React.RefObject<FormInstance>).current!.resetFields();
      onReset && onReset();
    };
