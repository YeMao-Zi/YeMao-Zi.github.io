...
```javascript
javascript复制代码import Dialog from './Dialog'
...
>
> ```markdown
> markdown复制代码  1. 提高性能，压缩体积。模块化开发有很多引用关系，打包后会变成一个js，减少了http请求。
...
- 第一步：设置`package.json`中的配置项
  ...
  - 我们可以添加其他个人信息：
    ...
    js复制代码  "name": "lxj-ui",
      "version": "0.0.1",
      "private": false,
      "main": "dist/lxj-ui.umd.min.js",
      "author": {
        "name": "liuxueji",
        "github":"https://github.com/liuxueji/lxj-ui"
      },
    ```
...
- 第三步：上传到`npm`
  ...
  - 如果安装了`nrm`，需要保证当前的源是`npm`，不能是淘宝源镜像
    ...
    ```arduino
    arduino
    复制代码npm config set registry=https://registry.npmjs.org
    ```
...
- 将组件下载到本地项目
  ...
  ```less
  less
  复制代码npm i lxj-ui //将我们包下载下来，就可以在项目的node-modules路径下找到这个包了
  ```
...
- 使用`lxj-ui`
  ...
  - 在main中导入组件和组件依赖的样式
    ...
    ```javascript
    javascript复制代码import LxjUi from 'lxj-ui' 
