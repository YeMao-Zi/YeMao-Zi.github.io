...
```mermaid
graph LR
    A["项目根目录/CLAUDE.md"] --> E[全局项目配置]
    B["src/.claude/CLAUDE.md"] --> F[模块级配置]
    C[".claude/rules/auth.md"] --> G[特定规则]
    D["~/.claude/CLAUDE.md"] --> H[用户级配置]
    style A fill:#667eea
    style B fill:#764ba2
    style C fill:#f093fb
    style D fill:#f5576c
...
```mermaid
graph LR
    A["~/.claude/CLAUDE.md"] --> E[合并规则]
    B["项目级/CLAUDE.md"] --> E
    C["src/.claude/CLAUDE.md"] --> E
    D[".claude/rules"] --> E
    E --> F[最终配置]
    style A fill:#f5576c
    style B fill:#667eea
