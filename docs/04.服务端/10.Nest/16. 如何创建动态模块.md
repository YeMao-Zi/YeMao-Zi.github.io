...
```
![](https://s2.loli.net/2025/03/21/8ew9xDWBy7NgRhA.png)
...
![](https://s2.loli.net/2025/03/21/u2WiKNdSyImkBGP.png)
...
![](https://s2.loli.net/2025/03/21/V2zRWbw5kvSjfFg.png)

这个模块是静态的，也就是它的内容是固定不变的，每次 import 都是一样：

![](https://s2.loli.net/2025/03/21/pTMQWbsijw1N4Bf.png)

![](https://s2.loli.net/2025/03/21/dRMme26wsoEnpau.png)

有的时候我们希望 import 的时候给这个模块传一些参数，动态生成模块的内容，怎么办呢？

## Dynamic Module 生成动态模块（register）
...
![](https://s2.loli.net/2025/03/21/AMchLybjXmWdPkU.png)

![](https://s2.loli.net/2025/03/21/j6ohGFylBzZOfdL.png)

而且我们还可以把参数传入的 options 对象作为一个新的 provider。

import 的时候就得这样用了，通过 register 方法传入参数，返回值就是模块定义：

![](https://s2.loli.net/2025/03/21/SUnxBsMK2NkCVgb.png)

改成这样之后，再跑一下：

![](https://s2.loli.net/2025/03/21/GvdeHaQrZRO6sX9.png)

依然是正常的。

而且这时候我们把传入的 options 通过 useValue 创建了个 provider，这样模块内部就可以注入它了。

![](https://s2.loli.net/2025/03/21/sRZpfXanIE6BgYc.png)

我在 BbbController 里面通过 token 注入这个 provider，打印下它的值。

改一下 register 的参数：

![](https://s2.loli.net/2025/03/21/EC7Ql8mtRVurNwW.png)

浏览器再访问下，可以看到控制台打印了 config 对象：

![](https://s2.loli.net/2025/03/21/kCU5xNgPw1jdEIf.png)
...
![](https://s2.loli.net/2025/03/21/T4PzLZdRAbopsGj.png)

forRoot 传入配置，动态产生 provider 和 exports，返回模块定义。

而且还有 forRootAsync：

![](https://s2.loli.net/2025/03/21/C7Er9DRlgZPvIi8.png)

区别就是可以用 async 的 useFactory 动态产生 provider，比如异步请求别的服务拿到配置返回，作为 options。

forFeature 则是传入局部的一些配置，来动态产生局部用的模块：

![](https://s2.loli.net/2025/03/21/2HE4aUQC8AmWIx6.png)

typeorm 的模块用起来是这样的：

![](https://s2.loli.net/2025/03/21/JZR6ig41hIFkYb2.png)

![](https://s2.loli.net/2025/03/21/4amb9Qu5d8MSinv.png)
...
![](https://s2.loli.net/2025/03/21/a1J4qpf5zhdLGWo.png)

这样这个 CccModule 就已经有了 register 和 registerAsync 方法了：

![](https://s2.loli.net/2025/03/21/NarTtm47kvM6pDX.png)

不用自己定义了，省事了不少。

传入 options：

![](https://s2.loli.net/2025/03/21/ORKVcQmkxECpjf1.png)

那现在如何在 Module 内注入这个 options 呢？

记得 build class 的时候返回了一个 token 么？

![](https://s2.loli.net/2025/03/21/ZnQsIHkJtTcrvDS.png)

就用这个注入：

![](https://s2.loli.net/2025/03/21/RTjCLGOKPvzMoEe.png)
...
![](https://s2.loli.net/2025/03/21/HY8ShRqv9OjTsPx.png)
...
![](https://s2.loli.net/2025/03/21/2boAxln1mX4EIYF.png)
...
![](https://s2.loli.net/2025/03/21/PyBG6qcYDUEOfrd.png)

![](https://s2.loli.net/2025/03/21/JTLM87lKkghp6r9.png)
...
![](https://s2.loli.net/2025/03/21/3c5jpYFM2mrBAJt.png)
...
![](https://s2.loli.net/2025/03/21/6ZN4MsR3KTJj7Ar.png)

最好是用 builder 返回的类型：

![](https://s2.loli.net/2025/03/21/7SoEjWvrhVURkeg.png)

这样就有了：

![](https://s2.loli.net/2025/03/21/gpnQNVS32s5Yd6r.png)

而这个 ASYNC\_OPTIONS\_TYPE 是 async 方式创建模块的 otpion 类型：

![](https://s2.loli.net/2025/03/21/y4C6IFlJeu1iVcj.png)
