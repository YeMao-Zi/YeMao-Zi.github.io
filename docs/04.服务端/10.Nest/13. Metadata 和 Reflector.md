---
title: Metadata和Reflector
date: 2025-03-10 14:24:01
permalink: /pages/nestmetaandreflector/
categories:
  - 技术
  - Node
  - Nest
...
![](https://s2.loli.net/2025/03/19/MIKxRmgo1apy3Wh.png)

`Reflect.set` 是设置对象属性值

![](https://s2.loli.net/2025/03/19/H5FqRL7pkt1sjMy.png)

`Reflect.has` 是判断对象属性是否存在

![](https://s2.loli.net/2025/03/19/nSokNpU2drsBm9A.png)

`Reflect.apply` 是调用某个方法，传入对象和参数

![](https://s2.loli.net/2025/03/19/xKjMvyCiIsAbBQu.png)

`Reflect.construct` 是用构造器创建对象实例，传入构造器参数

![](https://s2.loli.net/2025/03/19/zEGkDLgI6Cbhu7O.png)
...
![](https://s2.loli.net/2025/03/19/4c7a5Hw9ZRNg6F3.png)
...
![](https://s2.loli.net/2025/03/19/D4vFzNqrS6ju5QO.png)
...
![](https://s2.loli.net/2025/03/19/9lcfRX7EOqvkJTA.png)

![](https://s2.loli.net/2025/03/19/2eqgKskOXbhyU8C.png)

而且 @Controller 和 @Injectable 的装饰器也是这样实现的：

![](https://s2.loli.net/2025/03/19/Hrn2XL5dYvCwEIO.png)

![](https://s2.loli.net/2025/03/19/MTyGVcCn4Y2is1b.png)
...
![](https://s2.loli.net/2025/03/19/C94QwSN5m7Yz1po.png)

但是呢，ts 有一个编译选项叫做 `emitDecoratorMetadata`，开启它就会自动添加一些元数据。

开启之后再试一下：

![](https://s2.loli.net/2025/03/19/dlJuSpny72C1YIP.png)
...
![](https://s2.loli.net/2025/03/19/5G3WjAQOv8lyFJ6.png)

就是获取构造器的参数类型的。这个常量就是我们上面说的那个：

![](https://s2.loli.net/2025/03/19/ip7nWRgMmQJSFXv.png)

这也是为什么 `nest` 会用 `ts` 来写，因为它很依赖这个 `emitDecoratorMetadata` 的编译选项。

你用 `cli` 生成的代码模版里也都默认开启了这个编译选项：

![](https://s2.loli.net/2025/03/19/I4Sgl9qbHxPRNjs.png)
...
![](https://s2.loli.net/2025/03/19/8cZyqtJbCsEfgAu.png)

这个装饰器的底层实现自然是 `Reflect.defineMetadata`：

![](https://s2.loli.net/2025/03/19/ZjIChX9HK8O2x5s.png)
...
![](https://s2.loli.net/2025/03/19/yLqpS2k1UxsnlFd.png)

在路由级别应用：

![](https://s2.loli.net/2025/03/19/qBkj7F9QIDTO3oH.png)

加个打印语句：

![](https://s2.loli.net/2025/03/19/UtkDxomZn2lG8KT.png)

![](https://s2.loli.net/2025/03/19/H4oUwpsG5BT9Of8.png)

然后 nest start --watch 把服务跑起来。

浏览器访问：

![](https://s2.loli.net/2025/03/19/KLUhrY6DM4Sca7Z.png)

可以看到 guard 和 interceptor 成功执行了：

![](https://s2.loli.net/2025/03/19/hn51P9ldiJatoC6.png)

然后我们用 `@SetMetadata` 在 `controller` 上加个 `metadata`：

![](https://s2.loli.net/2025/03/19/81w6fM9RSWdDIam.png)

在 `guard` 和 `interceptor` 就就可以这样取出来：

## reflector

![](https://s2.loli.net/2025/03/19/XRSBmF8k4JQexAr.png)

通过 `ExecutationContext` 取到目标 `handler`，然后注入 `reflector`，通过 `reflector.ge`t 取出 `handler` 上的 `metadata`。

`interceptor` 里也是这样，这里换种属性注入方式：

![](https://s2.loli.net/2025/03/19/ckLPxCnF5JDlvwt.png)

刷新下页面，就可以看到已经拿到了 `metadata`：

![](https://s2.loli.net/2025/03/19/uNm4ZBX2hrfMA1b.png)
...
![](https://s2.loli.net/2025/03/19/lVZ7qKsYESd2phc.png)

![](https://s2.loli.net/2025/03/19/jfwEx3qX9k8vN2V.png)

![](https://s2.loli.net/2025/03/19/c6xbo5gslQECi8h.png)

### reflector 的 4 个方法

reflector 还有 3 个方法：

![](https://s2.loli.net/2025/03/19/COR42DmPlJdN9wu.png)
...
![](https://s2.loli.net/2025/03/19/SEdJaFcZwyvCjK6.png)

get 的实现就是 `Reflect.getMetadata`。

![](https://s2.loli.net/2025/03/19/ZXdEhUp8H92acJQ.png)

`getAll` 是返回一个 `metadata` 的数组。

![](https://s2.loli.net/2025/03/19/Zsxl7IpdoGzhftv.png)

`getAllAndMerge`，会把它们合并为一个对象或者数组。

![](https://s2.loli.net/2025/03/19/kdIFSXivpLYV4hW.png)

`getAllAndOverride` 会返回第一个非空的 `metadata`。

我们试一下：

![](https://s2.loli.net/2025/03/19/xlcp4T8MiHE9Y3v.png)

可以看到它们结果的区别：

![](https://s2.loli.net/2025/03/19/qg8fbWupk4621cS.png)
