...
### 5.返回 yyyy-mm-dd hh:mm:ss 格式时间

getFullYear()方法可返回一个四位数年份。

getUTCFullYear()函数则是根据 UTC 时间返回了四位数来代表年份。

一般情况下 getUTCFullYear 和 getFullYear 返回的结果相同，但是假如当天日期是12月31日或1月1日，两者可能会有差值。

比如在中国大陆、香港、澳门、蒙古国、台湾、新加坡、马来西亚、菲律宾等地区的本地时间比UTC快8小时，记作UTC+8，意思就是比UTC时间快8小时。减的类似理解，比如 UTC-10 等。
...
### 4.按字符串比对排序(如：按日期倒序排序)

```js
const Date=['2021-11-11','2022-09-11','2020-08-08']
const Date2=Date.slice().sort((a, b) => b.localeCompare(a))
```
...
console.log(getParams(window.location.search,'redirect_url'));
```

2.URLSearchParams 方法（可能存在兼容性问题）
...
export const removeEmptyObjParams = (
  ...
  return Object.keys(obj)
    .filter((key) => !emptyTypes.includes(obj[key]))
