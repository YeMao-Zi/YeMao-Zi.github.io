...
const getSessionStorage = (itemName) => {
  try {
    ...
    console.error(`Failed to parse sessionStorage item "${itemName}":`, error);
    return null;
  }
};

/**
 * 向 sessionStorage 中设置数据
 * @param {string} itemName - 存储项的键名
...
const setSessionStorage = (itemName, value) => {
  try {
    ...
    console.error(`Failed to set sessionStorage item "${itemName}":`, error);
  }
};
 ...
 * @param {Function} options.set - 设置值时的回调函数
 * @param {Function} options.get - 获取值时的回调函数
 * @returns {Proxy} 带有持久化功能的代理对象
 */
export const usePersist = (key, data, options) => {
  ...
  const { set: setCallback, get: getCallback } = options || {};
  ...
  const proxyData = new Proxy(store, {
    set(target, property, value, receiver) {
      // 执行设置回调
      if (setCallback) {
        setCallback(target, property, value, receiver);
      }
    ...
    get(target, property, receiver) {
      // 执行获取回调
      if (getCallback) {
        getCallback(target, property, receiver);
      }
